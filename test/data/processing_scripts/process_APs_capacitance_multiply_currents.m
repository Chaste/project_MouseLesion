...
for t = 1:length(cell_types)
    ...
    for c = 1:length(conductivities)
        ...
        % conductivity etc.
        data = load('../results_capacitance_multiply_current_10​/Myocyte_scar_cond_1_cap_1_boundary_cond_1_period_100_end_100.dat');
        times = data(:,1) - 5;
        ...
        for i=1:length(neutral_capacitances)
            figure(a)
            data = load(['../results_capacitance_multiply_current_10/' cell_types{t}  '_scar_cond_' num2str(conductivities(c))...
                '_cap_' num2str(neutral_capacitances(i)) '_boundary_cond_1_period_100_end_100.dat']);
        ...
        for i=1:length(neutral_capacitances)
            recovering = importdata(['../results_capacitance_multiply_current_10/' cell_types{t} '_scar_cond_' num2str(conductivities(c))...
                '_cap_' num2str(neutral_capacitances(i)) '_boundary_cond_1_period_100_end_100_APs.dat']);
            
            % The scar region in early recovery - let's say conductance is
            % 0.001
            early = importdata(['../results_capacitance_multiply_current_10/' cell_types{t} '_scar_cond_0.001_cap_1_boundary_cond_1_period_100_end_100_APs.dat']);
